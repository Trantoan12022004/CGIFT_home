...
const DISCOUNT_SYSTEM = {
    ...
    generateDiscountForOrder(orderValue) {
        ...
        if (orderValue >= 500000) {
            ...
            availableDiscounts = availableDiscounts.filter(d => 
                (d.type === 'percentage' && d.value >= 15) ||
                (d.type === 'fixed' && d.value >= 40000) ||
                d.type === 'shipping'
            );
        } else if (orderValue >= 300000) {
            // Đơn từ 300k: loại bỏ discount thấp nhất
            availableDiscounts = availableDiscounts.filter(d => 
                !(d.type === 'percentage' && d.value === 10) &&
                !(d.type === 'fixed' && d.value === 25000)
            );
        }
