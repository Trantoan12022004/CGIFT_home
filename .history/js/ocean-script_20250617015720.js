...
const DISTANCE_CONFIG = {
    ...
    REFERENCE_POINTS: {
        ...
        "nam từ liêm": { lat: 21.013333, lng: 105.765000 },
        "bắc từ liêm": { lat: 21.070000, lng: 105.770000 },
    ...
    try {
        ...
        if (data.results && data.results.length > 0) {
            ...
            for (const result of data.results) {
                if (result.formatted.toLowerCase().includes("hà nội") || 
                    result.formatted.toLowerCase().includes("hanoi")) {
                    bestResult = result;
                    break;
                }
            }
...
function calculateEnhancedDistance(lat1, lon1, lat2, lon2) {
    ...
    if (lat2 > 21.05) { // Phía Bắc Hà Nội (xa trung tâm hơn)
        adjustmentFactor = 1.4;
    } else if (lat2 < 20.95) { // Phía Nam Hà Nội  
        adjustmentFactor = 1.35;
    } else if (Math.abs(lon2 - 105.85) > 0.05) { // Xa trung tâm theo kinh độ
