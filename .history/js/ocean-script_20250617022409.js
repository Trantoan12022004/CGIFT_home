...
async function calculateDistanceFallback(address) {
    try {
        ...
        if (data && data.length > 0) {
            ...
            // Sử dụng hàm calculateHaversineDistance đã có hiệu chỉnh +3km
            const distance = calculateHaversineDistance(hustLat, hustLng, lat, lng);
...
// Cải tiến hàm Haversine với hiệu chỉnh sai số
function calculateHaversineDistance(lat1, lon1, lat2, lon2) {
    ...
    // Hiệu chỉnh sai số: cộng thêm 3km để phản ánh khoảng cách thực tế
    distance = distance + 3;
    ...
    try {
        ...
        if (result && result.distance !== null) {
            ...
            if (distance <= 5) {
                shippingFee = 0;
                distanceText.innerHTML = `
                    <div style="background: linear-gradient(145deg, #f0fdf4, #dcfce7); padding: 10px; border-radius: 8px; border-left: 4px solid #10b981;">
                        <strong>📍 Khoảng cách: ${distance.toFixed(1)}km</strong>
                        ${duration ? `<br><small>⏱️ Thời gian: ~${Math.round(duration)} phút</small>` : ""}
                        <br><small style="color: #6b7280;">Đã hiệu chỉnh sai số (+3km) cho chính xác hơn</small>
                    </div>
                `;
                ...
                distanceText.innerHTML = `
                    <div style="background: linear-gradient(145deg, #f0f9ff, #e0f7fa); padding: 10px; border-radius: 8px; border-left: 4px solid #0ea5e9;">
                        <strong>📍 Khoảng cách: ${distance.toFixed(1)}km</strong>
                        ${duration ? `<br><small>⏱️ Thời gian: ~${Math.round(duration)} phút</small>` : ""}
                        <br><small style="color: #6b7280;">Đã hiệu chỉnh sai số (+3km) cho chính xác hơn</small>
                    </div>
...
// Tính khoảng cách từ HUST (Đại học Bách khoa Hà Nội) - có hiệu chỉnh sai số
async function calculateDistance(address) {
    try {
        // Tọa độ HUST: 21.0054;
        ...
        if (data && data.length > 0) {
            ...
            // Tính khoảng cách bằng công thức Haversine (đã có hiệu chỉnh +3km)
            const distance = calculateHaversineDistance(hustLat, hustLng, lat, lng);
            return distance;
        } else {
...
// Cập nhật phí giao hàng dựa trên khoảng cách - hiển thị thông tin hiệu chỉnh
async function updateShippingFee() {
    const addressInput = document.getElementById("address");
    const distanceInfo = document.getElementById("distanceInfo");
    const distanceText = document.getElementById("distanceText");
