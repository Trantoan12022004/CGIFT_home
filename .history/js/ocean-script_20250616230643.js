...
async function calculateDistance(address) {
    try {
        ...
        const response = await fetch(
            `https://nominatim.openstreetmap.org/search?` + `format=json&` + `q=${encodedQuery}&` + `limit=3&` + `addressdetails=1&` + `countrycodes=vn&` + `accept-language=vi`,
            {
                headers: {
...
async function calculateDistanceSimple(address) {
    try {
        ...
        const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(simpleQuery)}&limit=1&countrycodes=vn`, {
            headers: {
                "User-Agent": "CTES-SIE-SHOP-Website",
            },
        });
