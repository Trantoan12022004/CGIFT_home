...
async function updateShippingFee() {
    ...
    try {
        ...
        if (result && result.distance !== null) {
            ...
            const distance_new = ddistance+2;
            // Tính phí ship
            if (distance_new <= 5) {
                shippingFee = 0;
                distanceText.innerHTML = `
                    <div class="distance-result success">
                        <strong>📍 Khoảng cách: ${distance_new.toFixed(1)}km</strong>
                ...
                shippingFee = Math.ceil(distance_new - 5) * 5000;
                distanceText.innerHTML = `
                    <div class="distance-result">
                        <strong>📍 Khoảng cách: ${distance_.toFixed(1)}km</strong>
                        ${duration ? `<br><small>⏱️ Thời gian: ~${Math.round(duration)} phút</small>` : ""}
                        <br><small class="method-info">${methodText}</small>
                    </div>
                `;
