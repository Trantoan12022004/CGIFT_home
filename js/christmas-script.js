...
// Discount codes database - Christmas Edition
const discountCodes = {
    XMAS2025: {
        type: "percentage",
        value: 10,
    ...
    NOEL2025: {
        ...
        maxDiscount: 25000,
        minOrder: 150000,
        description: "Giảm 25k cho đơn hàng Giáng sinh",
    },
    FREESHIP: {
        type: "shipping",
        value: 100,
        ...
        <div class="product-card ${cardClass}" id="product${product.id}">
            ...
            <div class="product-info">
                ...
                <div class="price-section">
                    ${originalPriceHTML}
                    <span class="current-price">${product.price.toLocaleString("vi-VN")}đ/${
        product.unit
    }</span>
                </div>
                <div class="order-section">
                    <input type="number" id="${
                        product.quantityInputId
                    }" class="quantity-input" min="1" max="50" value="1" placeholder="SL">
...
// Filter products - Christmas Edition
function filterProducts(filter) {
    ...
    // Show loading animation - Christmas themed
    const productContainer = document.querySelector(".products-grid");
...
// Hiệu ứng thêm vào giỏ hàng - Christmas colors
function showAddToCartAnimation(productId) {
    const productCard = document.getElementById(`product${productId}`);
    if (productCard) {
        productCard.style.transform = "scale(1.05)";
        productCard.style.boxShadow = "0 20px 40px rgba(196, 30, 58, 0.6)";
...
function applyDiscountCode() {
    ...
    if (discountCodes[code]) {
        ...
        if (currentSubtotal < discount.minOrder) {
            statusDiv.className = "discount-status error";
            statusDiv.textContent = `⚠️ Đơn hàng tối thiểu ${discount.minOrder.toLocaleString(
                "vi-VN"
            )}đ để sử dụng mã này`;
            statusDiv.style.display = "block";
            return;
...
function updatePaymentSummary() {
    ...
    document.getElementById("shippingFeeDisplay").textContent =
        "freeship bán kính 5km đầu quanh Bách Khoa, sau 5km đầu là 5k mỗi 1km";
...
function handlePaymentMethodChange() {
    ...
    if (paymentMethod === "bank") {
        ...
        const finalTotal =
            cart.reduce((total, item) => total + item.totalPrice, 0) + shippingFee - discountAmount;
        updateVietQR(finalTotal);
    } else {
...
// Cập nhật VietQR - Christmas Edition
function updateVietQR(amount) {
    ...
    if (amount > 0) {
        ...
        // Tạo ID đơn hàng - Christmas Edition
        const now = new Date();
        const ddMMyy =
...
// Cập nhật form thanh toán - Christmas themed
function updatePaymentForm() {
    ...
    if (cart.length === 0) {
        cartItemsList.innerHTML =
            '<li class="empty-cart">🎄 Giỏ hàng trống - Hãy thêm sản phẩm yêu thích!</li>';
    ...
    cart.forEach((item, index) => {
        ...
        listItem.innerHTML = `
            <div class="cart-item-info">
                ...
                <div class="item-details">Số lượng: ${
                    item.quantity
                } | Đơn giá: ${item.price.toLocaleString("vi-VN")}đ</div>
                <div class="item-total">Thành tiền: ${item.totalPrice.toLocaleString(
                    "vi-VN"
                )}đ</div>
            </div>
...
// Gửi đơn hàng - Christmas Edition
function submitOrder() {
    ...
    let cartItemsString = "";
    cart.forEach((item, index) => {
        cartItemsString += `Sản phẩm ${index + 1}: ${item.productName}, SL: ${
            item.quantity
        }, Giá: ${item.price.toLocaleString("vi-VN")}đ, Tổng: ${item.totalPrice.toLocaleString(
            "vi-VN"
        )}đ; `;
    });
    ...
    const orderData = {
        ...
        campaign: "XMAS2025", // Christmas campaign identifier
    };

    // Gửi dữ liệu
    fetch(
        "https://script.google.com/macros/s/AKfycbzRZQFPxw5kC3GWMbnHdRJnUuFhVZHqaLDgObgMPGk-wq5YFCJgFWppaT8mk2Nbe96R1g/exec",
        {
            method: "POST",
            mode: "no-cors",
        ...
        .then(() => {
            showNotification(
                "🎉 Đặt hàng thành công! Chúc bạn có mùa Giáng sinh ấm áp! 🎄",
                "success"
            );
